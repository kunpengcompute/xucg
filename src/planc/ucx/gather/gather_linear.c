...
static ucg_status_t ucg_planc_ucx_gather_linear_op_root(ucg_planc_ucx_op_t *op)
{
    ...
    if (ucg_test_and_clear_flags(&op->flags, UCG_GATHER_LINEAR_RECV)) {
        int64_t recvtype_extent = ucg_dt_extent(args->recvtype);
        for (int i = 0; i < group_size; i++) {
            void *rbuf = (char*)args->recvbuf + args->recvcount * i * recvtype_extent;
            if (args->recvcount == 0) {
                continue;
            }
            if (i == args->root) {
                ...
                status = ucg_planc_ucx_p2p_irecv(rbuf, args->recvcount, args->recvtype, i,
...
static ucg_status_t ucg_planc_ucx_gather_linear_op_non_root(ucg_planc_ucx_op_t *op)
{
    ucg_status_t status = UCG_OK;
    ucg_coll_gather_args_t *args = &op->super.super.args.gather;
    ucg_vgroup_t *vgroup = op->super.vgroup;
    ...
    if (ucg_test_and_clear_flags(&op->flags, UCG_GATHER_LINEAR_SEND)) {
        if (args->sendcount != 0) {
...
ucg_status_t ucg_planc_ucx_gather_linear_op_progress(ucg_plan_op_t *ucg_op)
{
    ...
    if (myrank == args->root) {
        status = ucg_planc_ucx_gather_linear_op_root(op);
    } else {
        status = ucg_planc_ucx_gather_linear_op_non_root(op);
    }
    op->super.super.status = status;
    return status;
}

static ucg_status_t ucg_planc_ucx_gather_linear_op_trigger(ucg_plan_op_t *ucg_op)
{
    ucg_status_t status;
    ucg_planc_ucx_op_t *op = ucg_derived_of(ucg_op, ucg_planc_ucx_op_t);
    ucg_planc_ucx_op_reset(op);
    op->flags = UCG_GATHER_LINEAR_FLAGS;
    status = ucg_planc_ucx_gather_linear_op_progress(ucg_op);
                                                        ...
                                                        const ucg_coll_args_t *args)
{
    ...
    ucg_status_t status = UCG_CLASS_CONSTRUCT(ucg_plan_op_t, &ucx_op->super, vgroup,
                                              ucg_planc_ucx_gather_linear_op_trigger,
                                              ucg_planc_ucx_gather_linear_op_progress,
...
static ucg_status_t ucg_planc_ucx_gather_linear_check(ucg_vgroup_t *vgroup)
{
    UCG_UNUSED(vgroup);
    return UCG_OK;
}
                                                  ...
                                                  ucg_plan_op_t **op)
{
    ...
    ucg_status_t status;
    status = ucg_planc_ucx_gather_linear_check(vgroup);
    if (status != UCG_OK) {
        return status;
    }
    ucg_planc_ucx_group_t *ucx_group = ucg_derived_of(vgroup, ucg_planc_ucx_group_t);
    ucg_planc_ucx_op_t *linear_op = ucg_planc_ucx_gather_linear_op_new(ucx_group, vgroup, args);
    if (linear_op == NULL) {
