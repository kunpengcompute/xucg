...
typedef enum {
    ...
    COLL_TYPE_ALLTOALLV,
    /*
    * Only collective oprations that already
...
typedef struct {
    ...
    uint16_t ppn_max;    /* number of processes on all nodes */
    uint16_t node_nums;
...
/**
 * @ingroup UCG_GROUP
 * @brief Cancel an outstanding commutications request.
 *
 * @param [in]  worker      UCG worker.
 * @param [in]  request     Non-blocking request to cancel.
 *
 * This routine tries to cancels an outstanding commutication request.
 */
 ...
 *
 * This routine release then non-blocking request back to the library, regardless
 * of its current state. Commutications operations associated with this request
