...
enum {
    ...
    UCG_KNTREE_ADJUST_ROOT_DATA = UCG_BIT(2),
    UCG_KNTREE_INTER_NODE_PARAMS = UCG_BIT(3),
    UCG_KNTREE_INTER_NODE_DATA = UCG_BIT(4),
    UCG_KNTREE_INTRA_NODE_PARAMS = UCG_BIT(5),
    UCG_KNTREE_INTRA_NODE_DATA = UCG_BIT(6),
    UCG_KNTREE_RECV_FROM_PARENT = UCG_BIT(7),
    UCG_KNTREE_SEND_TO_CHILD = UCG_BIT(8),
    UCG_KNTREE_SEND_SENDCOUNT = UCG_BIT(9),
    UCG_KNTREE_SEND_DTSIZE = UCG_BIT(10),
    UCG_KNTREE_SEND_SENDBUF = UCG_BIT(11),
    UCG_KNTREE_RECV_SENDCOUNT = UCG_BIT(12),
    UCG_KNTREE_RECV_DTSIZE = UCG_BIT(13),
    UCG_KNTREE_RECV_SENDBUF = UCG_BIT(14),
};
...
static ucg_status_t ucg_planc_ucx_scatter_inter_node_params_op_prepare(ucg_plan_op_t *ucg_op)
{
    ...
    ucg_coll_scatter_args_t *args = &ucx_op->scatter.na_kntree.scatter_origin.scatter;
    ucg_algo_kntree_iter_t *iter = &ucx_op->scatter.na_kntree.kntree_iter;
...
static ucg_status_t ucg_planc_ucx_scatter_adjust_root_data_op_prepare(ucg_plan_op_t *ucg_op)
{
    ...
    if (myrank == UCG_TOPO_GROUP_LEADER) {
        na_args->sdtype_size = ucg_dt_size(args->sendtype);
        na_args->sendbuf_size = args->sendcount * na_args->global_nprocs * na_args->sdtype_size;
        /* rank 0 set sendbuf size */
        uint64_t bufsize = na_args->sendbuf_size;
...
static ucg_status_t ucg_planc_ucx_scatter_na_kntree_op_trigger(ucg_plan_op_t *ucg_op)
{
    ...
    if (ucg_op->vgroup->myrank == args->root) {
        ...
        status = ucg_dt_memcpy((void*)inter_args->sendbuf, total_count, args->sendtype,
                                args->sendbuf, total_count, args->sendtype);
        /* set sendcount */
        inter_args->sendcount = args->sendcount;
    ...
    if (args->root != UCG_TOPO_GROUP_LEADER) {
        if (vgroup->myrank == UCG_TOPO_GROUP_LEADER) {
            ucx_op->flags = UCG_KNTREE_ADJUST_ROOT_RECV_DATA_FLAGS;
        } else if (vgroup->myrank == args->root) {
            ucx_op->flags = UCG_KNTREE_ADJUST_ROOT_SEND_DATA_FLAGS;
        }
    }
