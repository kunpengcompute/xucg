...
static void ucg_planc_ucx_gather_na_kntree_op_init(ucg_planc_ucx_op_t *ucx_op)
{
                                                        ...
                                                        const ucg_coll_args_t *args)
{
    ...
    ucg_planc_ucx_gather_na_kntree_op_init(ucx_op);
    return ucx_op;
                                                  ...
                                                  ucg_plan_op_t **op)
{
    ...
    ucg_status_t status;
    status = ucg_planc_ucx_gather_na_kntree_check(vgroup, args);
    if (status != UCG_OK) {
        return status;
    }
    ucg_planc_ucx_group_t *ucx_group = ucg_derived_of(vgroup, ucg_planc_ucx_group_t);
    ucg_planc_ucx_op_t *na_kntree_op = ucg_planc_ucx_gather_na_kntree_op_new(ucx_group, vgroup, args);
    if (na_kntree_op == NULL) {
        return UCG_ERR_NO_MEMORY;
    }
    *op = &na_kntree_op->super;
    return UCG_OK;
}
