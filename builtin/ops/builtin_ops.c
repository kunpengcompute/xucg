 ...
 * OUTPUT: fragment_length, fragment, flag
 */
   ...
   /*
     ...
     */
        ...
        if ((length < max_zcopy_one) && (max_zcopy_one != 0)) {
            ...
            *fragment_length = (dt_len > max_zcopy_one) ?
                                    max_zcopy_one:
                                    (max_zcopy_one - (max_zcopy_one % dt_len)) ;
            ucs_assert(*fragment_length > 0);
            extra_frag = (length % (*fragment_length)) > 0;
...
/* Add rank id to the front of variable-length operation */
                                                                      ...
                                                                      uct_ep_h ep, int is_single_send)
{
    ...
    status = ucg_builtin_ep_am_short_pack_rank(ep, am_id, am_iter.header, buffer_iter, send_buffer + step->buffer_length - buffer_iter, step);
...
static size_t ucg_builtin_step_am_bcopy_single_frag_packer(void *dest, void *arg)
{
    ...
    if (dt_state != NULL) {
        ...
        errno_t error_status = memcpy_s(header_ptr + header_len, step->buffer_length, step->send_buffer, step->buffer_length);
        if (error_status != EOK) {
            ucs_fatal("memcpy_s encountered an error. The error code is %d.", error_status);
        }
    }
...
static size_t ucg_builtin_step_am_bcopy_full_frag_packer(void *dest, void *arg)
{
    ...
    if (dt_state != NULL) {
        ...
        errno_t error_status = memcpy_s(header_ptr + header_len,  step->fragment_length, step->send_buffer + step->iter_offset, step->fragment_length);
        if (error_status != EOK) {
            ucs_fatal("memcpy_s encountered an error. The error code is %d.", error_status);
        }
    }
...
static size_t ucg_builtin_step_am_bcopy_partial_frag_packer(void *dest, void *arg)
{
    ...
    if (dt_state != NULL) {
        ...
        errno_t error_status = memcpy_s(header_ptr + header_len, last_frag_length, step->send_buffer + step->iter_offset, last_frag_length);
        if (error_status != EOK) {
            ucs_fatal("memcpy_s encountered an error. The error code is %d.", error_status);
        }
    }
                                                                      ...
                                                                      uct_ep_h ep, int is_single_send)
{
    ...
    if (step->phase->ex_attr.is_variable_len) {
        ...
        zcomp_index = step->iter_ep * step->fragments + step->iter_offset / step->fragment_length;
    }
    ...
    if (ucs_likely(iov.buffer < iov_buffer_limit)) {
        ...
        do {
            ...
            status = ucg_builtin_step_am_zcopy_pack_rank(ep, step, &iov,
                                 1, 0, &zcomp->comp);
            (zcomp++)->req = req;
...
static UCS_F_ALWAYS_INLINE ucs_status_t ucg_builtin_step_update_pending(ucg_builtin_request_t *req, unsigned recv_ep_index)
{
    ucs_status_t status = UCS_OK;
...
static ucs_status_t ucg_builtin_dynamic_send_recv(ucg_builtin_request_t *req, ucg_request_t **user_req)
{
    ...
    /* initialize the pending before both sending/receiving */
    if (step->resend_flag & UCG_BUILTIN_OP_STEP_FIRST_SEND) {
        req->pending = 0;
    }
    ...
    while (step->iter_ep < phase->send_ep_cnt) {
        ...
        if (*ep_iter) {
            ...
            if (step->send_coll_params->counts[block_idx] > 0) {
                ...
                /* calculate the the flag and fragments for sender */
...
#define case_send(req, ureq, step, phase, _send_func) do {                          \
        if ((is_rs1 || is_r1s) && ((step)->iter_ep == 0)) {                      \
            ...
            if (is_pipelined) {                                                  \
                memset_s((void*)(step)->fragment_pending, (step)->fragments,    \
                 ...
                 ucg_builtin_request_t *req, ucg_request_t **user_req)
{
    ...
    if (phase->ex_attr.is_variable_len) {
        ...
        ucs_debug("is_dummy:%d is_fragmented:%d is_short:%d is_bcopy:%d is_zcopy:%d", is_dummy, is_fragmented,is_short, is_bcopy, is_zcopy);
...
void *ucg_builtin_pack_rank(void *step, const void *send_buffer, size_t buffer_len, size_t *new_buffer_len)
{
    ...
    errno_t error_status = memcpy_s(temp_buffer, sizeof(ucg_group_member_index_t),
                                (int8_t *)&my_idx, sizeof(ucg_group_member_index_t));
    if (error_status != EOK) {
        ucs_fatal("memcpy_s encountered an error. The error code is %d.", error_status);
    }
    error_status = memcpy_s(temp_buffer + sizeof(ucg_group_member_index_t), buffer_len,
                                (int8_t *)send_buffer, buffer_len);
    if (error_status != EOK) {
        ucs_fatal("memcpy_s encountered an error. The error code is %d.", error_status);
    }
...
ucg_builtin_coll_params_t *ucg_builtin_allocate_coll_params(unsigned local_member_cnt)
{
    ...
    (void)memset_s(params->counts, local_member_cnt * sizeof(int), 0, local_member_cnt * sizeof(int));
    ...
    (void)memset_s(params->displs, local_member_cnt * sizeof(int), 0, local_member_cnt * sizeof(int));
    return params;
}
                                     ...
                                     ucg_builtin_op_step_t *step)
{
    ...
    (void)memset_s(step, sizeof(ucg_builtin_op_step_t), 0, sizeof(ucg_builtin_op_step_t));
    ...
    "ucg_zcopys_info");
    (void)memset_s(step->zcopys, phase->ep_cnt * sizeof(ucg_builtin_zcopy_info_t),
                   0, phase->ep_cnt * sizeof(ucg_builtin_zcopy_info_t));
    ...
    if (phase->method == UCG_PLAN_METHOD_ALLTOALLV_LADD) {
        ...
        step->resend_flag               = UCG_BUILTIN_OP_STEP_FIRST_SEND;
        step->am_header.remote_offset   = step->am_header.remote_offset;
                               ...
                               ucg_builtin_request_t *req)
{
    ...
    errno_t error_status = memcpy_s(tmp_buffer, length, netdata, length);
    if (error_status != EOK) {
        ucs_fatal("memcpy_s encountered an error, The error code is %d.", error_status);
    }
    if (gen_dt != NULL) {
        ...
        error_status = memcpy_s(netdata, length, recv_buffer + offset, length);
        if (error_status != EOK) {
            ucs_fatal("memcpy_s encountered an error, The error code is %d.", error_status);
        }
        error_status = memcpy_s(recv_buffer + offset, length, tmp_buffer, length);
        if (error_status != EOK) {
            ucs_fatal("memcpy_s encountered an error, The error code is %d.", error_status);
        }
    }
