                                                          ...
                                                          int isintel)
{
    ...
    ucg_planc_ucx_gather_config_t *config_gather = UCG_PLANC_UCX_CONTEXT_BUILTIN_CONFIG_BUNDLE(ucx_group->context, gather, UCG_COLL_TYPE_GATHER);;
    ucg_planc_ucx_bcast_config_t config_bcast;

    switch(algorithm) {
    case UCG_GATHER_KNTREE:
        if (isintel) {
            config_gather->kntree_degree = config_gather->na_kntree_inter_degree;
        } else {
            config_gather->kntree_degree = config_gather->na_kntree_intra_degree;
        }
        ucx_op = ucg_planc_ucx_gather_kntree_op_new(ucx_group, &topo_group->super, args, config_gather);
        break;
    case UCG_GATHER_BCAST_KNTREE:

        if (isintel) {
            config_bcast.kntree_degree = config_gather->na_kntree_inter_degree;
        } else {
            config_bcast.kntree_degree = config_gather->na_kntree_intra_degree;
        }
