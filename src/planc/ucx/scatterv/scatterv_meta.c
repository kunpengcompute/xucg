/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2026-2026. All rights reserved.
 */

#include "scatterv.h"
#include "core/ucg_group.h"
#include "core/ucg_topo.h"
#include "planc_ucx_meta.h"
#include "scatterv_meta.h"
#include "util/ucg_log.h"

ucg_planc_ucx_op_t *ucg_planc_ucx_scatterv_build_topo_group_op(ucg_planc_ucx_group_t *ucx_group,
                                                               ucg_vgroup_t *vgroup,
                                                               const ucg_coll_args_t *args,
                                                               ucg_base_algorithm_scatterv_t algorithm,
                                                               ucg_topo_group_type_t type)
{
    ucg_planc_ucx_op_t *ucx_op = NULL;
    ucg_planc_ucx_scatterv_config_t *config;
    ucg_topo_group_t *topo_group;

    config = UCG_PLANC_UCX_CONTEXT_BUILTIN_CONFIG_BUNDLE(ucx_group->context, scatterv,
                                                         UCG_COLL_TYPE_SCATTERV);
    
    topo_group = ucg_topo_get_group(vgroup->group->topo, type);
    if (topo_group == NULL) {
        return NULL;
    }

    if (topo_group->state == UCG_TOPO_GROUP_STATE_DISABLE) {
        /* I'm not in the topo group. */
        ucx_op = ucg_planc_ucx_empty_op_new(ucx_group, vgroup, args);
        return ucx_op;
    }

    if (topo_group->state != UCG_TOPO_GROUP_STATE_ENABLE) {
        /* The group state is incorrect. */
        return NULL;
    }

    switch (algorithm) {
    case UCG_SCATTERV_LINEAR:
        ucx_op = ucg_planc_ucx_scatterv_linear_op_new(ucx_group, &topo_group->super, args, config);
        break;
    case UCG_SCATTERV_KNTREE:
        ucx_op = ucg_planc_ucx_scatterv_kntree_op_new(ucx_group, &topo_group->super, args, config);
        break;
    case UCG_SCATTERV_LINEAR_SM:
        ucx_op = ucg_planc_ucx_scatterv_sm_op_new(ucx_group, &topo_group->super, args);
        break;
    default:
        ucg_error("Do not support scatterv algorithm!");
        return NULL;
    }

    return ucx_op;
}