...
STATIC_GTEST ucs_status_t ucg_builtin_get_recv_block_index(unsigned ep_cnt, unsigned ep_idx, unsigned* ret)
{
    ...
    errno_t res = memset_s(arr, alloc_size, 0, alloc_size);
    if (res != EOK) {
        ucs_free(arr);
        arr = NULL;
                                                        ...
                                                        ucg_builtin_index_group_t *index_group)
{
    ...
    *step_idx         = binary_block->step_cnt;
    ...
    /* 1st part: intra_node/socket reduce-scatter, use local index */
                                                         ...
                                                         const ucg_group_member_index_t member_cnt)
{
    ...
    if (status != UCS_OK) {
        ucs_error("Binary-blocks algorithm failed in intra reduce_scatter phase");
        return status;
    }
    binary_block->step_cnt = step_idx;

    /* 2nd part: intra_node/socket rabenseifner extra reduction */
    phase = &binary_block->phss[binary_block->phs_cnt];
                                                                    ...
                                                                    const ucg_group_member_index_t member_cnt)
{
    ...
    if (status != UCS_OK) {
        ucs_error("Binary-blocks topology-aware algorithm failed in intra reduce_scatter phase");
        return status;
    }
    binary_block->step_cnt = step_idx;
                                             ...
                                             ucg_builtin_plan_t **plan_p)
{
    ...
    ucg_builtin_plan_t *binary_block = (ucg_builtin_plan_t*)UCS_ALLOC_CHECK(alloc_size, "rabenseifner algorithm");

    /* Initialize all variables of current plan */
    errno_t res = memset_s(binary_block, alloc_size, 0, alloc_size);
    if (res != EOK) {
        ucs_free(binary_block);
        binary_block = NULL;
        return UCS_ERR_INVALID_PARAM;
    }
    /**
    ...
    if (binary_block->phs_cnt > MAX_PHASES) {
        ucs_error("Please increase the MAX_PHASES!\n");
    }
    if (binary_block->ep_cnt > MAX_PEERS) {
        ucs_error("Please increase the MAX_PEERS!\n");
    }
