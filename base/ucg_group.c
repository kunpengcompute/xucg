                            ...
                            struct ucg_group *new_group)
{
    ...
    errno_t status = memcpy_s(new_group->params.node_index, nodenumber_size, params->node_index, nodenumber_size);
    if (status != EOK) {
        return UCS_ERR_INVALID_PARAM;
    }
    status = memset_s(new_group + 1, ctx->total_planner_sizes, 0, ctx->total_planner_sizes);
    if (status != EOK ) {
        return UCS_ERR_INVALID_PARAM;
    }
...
void ucg_group_destroy(ucg_group_h group)
{
     ...
     * subroot send kill to root and root reply kill to suroot clear switch
     */
    ucs_status_t status;
